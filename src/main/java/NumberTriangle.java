...
public class NumberTriangle {
    ...
    public static NumberTriangle loadTriangle(String fname) throws IOException {
        ...
        if (!rows.isEmpty()) {
            top = new NumberTriangle(rows.get(0).get(0));
            List<NumberTriangle> prev = new ArrayList<>();
            prev.add(top);

            for (int i = 1; i < rows.size(); i++) {
                List<Integer> vals = rows.get(i);
                List<NumberTriangle> curr = new ArrayList<>();

                for (int j = 0; j < vals.size(); j++) {
                    NumberTriangle node = new NumberTriangle(vals.get(j));
                    curr.add(node);

                    if (j == 0) {
                        prev.get(0).setLeft(node);
                    } else if (j == vals.size() - 1) {
                        prev.get(j - 1).setRight(node);
                    } else {
                        prev.get(j - 1).setRight(node);
                        prev.get(j).setLeft(node);
                    }
                }
                prev = curr;
            }
